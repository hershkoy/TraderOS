...
def test_nightly_cli_defaults_match_l3_keeper():
    ...
    assert ns.max_rsi == LIVE_DEFAULTS["max_rsi"] == 50.0
    assert ns.require_in_channel is True
    assert ns.max_channel_span_days == 365.0
    assert ns.max_beyond_width == 0.25
    assert ns.atr_stop_mult == 2.0
    assert ns.max_entries_per_day == 1
