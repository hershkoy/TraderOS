#!/usr/bin/env python3
"""Install / start / stop the /hot price hub Windows logon task.

Examples (Windows CMD):
  venv\\Scripts\\activate && set PYTHONPATH=. && python scripts\\pipeline\\hot_price_service.py install-task
  python scripts\\pipeline\\hot_price_service.py start
  python scripts\\pipeline\\hot_price_service.py task-status
"""
...
def cmd_install_task(args: argparse.Namespace) -> int:
    ...
    print("It runs pythonw hot_price_server.py --service (no CMD window).")
    print("WS: ws://localhost:%s/ws/hot-candidates" % args.port)
    print("Logs: logs\\hot_price\\hot_price.log")
    print()
    if not args.no_start:
        start_windows_task()
        print("Started. /hot page reads live prices from port %s" % args.port)
        print()
    print("Useful:")
    print("  python scripts\\pipeline\\hot_price_service.py task-status")
    print("  python scripts\\pipeline\\hot_price_service.py start")
    print("  python scripts\\pipeline\\hot_price_service.py stop")
    print("  python scripts\\pipeline\\hot_price_service.py uninstall-task")
    return 0
...
def cmd_start(_args: argparse.Namespace) -> int:
    ...
    print("WS: ws://localhost:%s/ws/hot-candidates" % DEFAULT_PORT)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Hot price hub Windows logon task (always-on, not crontab.yaml)"
    )
    ...
    p_stop = sub.add_parser("stop", help="Stop the running price hub")
