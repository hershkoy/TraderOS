...
**Latest (2026-09-04):** `--realistic-fill` default is now **signal-bar close** (old next-bar mid kept as `--realistic-fill-mode next-mid`). Wait-1 300-name book **n=1777 E −0.12 PF 0.83** vs next-mid **n=1771 E +0.04 PF 1.06** — worse; do not promote. GOOGL 2025-11-18 BUY moves to the 15:00 UTC tag close 280.79; the 11:00 ET wick (16:00 UTC low 278.20, next mid 284) is a later tag wait-1 already skipped. See [signal-close](edge_hunt/channel_touch/2026-09-04_channel_touch_signal_close.md). Same day: L3 fills cannot print below support. See [no buy below](edge_hunt/channel_touch/2026-09-04_channel_touch_no_buy_below.md). IB 5m ingest: [5m backfill](edge_hunt/channel_touch/2026-09-04_ib_5m_backfill.md). **Keep wait-12** (next-mid **n=1746 E +0.09 PF 1.16**, not yet rescanned at signal-close). `--realistic-fill` replaced `current_best/` on next-mid (live daily H2 unique **n=713 E +1.78 PF 1.72**). Nightly scanner still uses close fills. See [realistic fill](edge_hunt/channel_touch/2026-09-04_channel_touch_realistic_fill.md).
...
| Rejected | Hard ADV/ATR floors; SPY SMA50 alone; structure-exit bundle; H3 lower-40% geometry; `entry_mode=reclaim` until look-ahead fixed; ridge P&L confidence sizing on RS-top1; L3 shakeout rebuy (5/10/15 bars); in-channel / RSI 50 on H2 breakouts; 15m unique H1–H4 (RS floor, tight-break, ATR ceiling, quiet-day); full-day 15m name counts; wiring 15m H5 into nightly |
...
- **Not fishing further (same windows):** Edge-hunt Sharpe>1 on Phase 5/6/6c overlays; reclaim until look-ahead fixed; SPY SMA hard filter; lower-40% geometry; l3_touch max-age/max-wait caps; bb%B 0.2; 15m `min_close_loc` wick filter (leaky or lagged); 15m entry MLP as a hard gate; ridge P&L confidence sizing (RS-top1 OOS); copying daily beyond-0.25 onto 15m L3; daily `--intraday-fill 15m` hybrid until it beats wait-6/RSI-50 without fill-bar close; L3 shakeout rebuy (5/10/15 bars); L3 in-channel / RSI 50 on H2 breakouts; **15m H2 resist-break as a replacement for wait-12 L3 on the full IB 15m set**; **15m L3 min-wait 1 (textbook first-tag) vs wait-12 on realistic fill**; 15m unique H1–H4 (tight-break / RS floor / ATR ceiling / quiet-day); logistic-on-H5 as a hard gate; wiring any 15m book into nightly.
- **Next (optional):** Run `backfill_ib_15m_universe.py` with Gateway up; then dry-run `channel_touch_15m.py`. Bootstrap / max-open on the 15m H5 book; portfolio max-open; ADV-tiered friction.
