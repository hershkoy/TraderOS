...
| Shakeout then second H2 break (2026-09-05) | After a taken H2 resist-break, if price closes back inside and support holds, fill the next close above resistance (rebuy, not hold-through). **Not** L3 support-reclaim. **any-closed min_inside=1:** sleeve n=1439 E **+3.16** PF **2.32** (2020-21 E **+3.24** PF 2.39); combined unique n=4558 E **+2.62** PF **2.10** vs frozen unique H2 **n=3364 E +2.40 PF 2.02**. Complete history (2223 names, IB prefix from 2006 through 2026-09-03): sleeve **n=1445 E +3.14 PF 2.30**; combined **n=4582 E +2.64 PF 2.10**. SXI 2020-10-05 extra **+55%**. Hard-stop-only and min_inside=5 are weaker. **Soft-promote `--shakeout-breakout`; nightly not rewired.** See [shakeout breakout](edge_hunt/channel_touch/2026-09-05_channel_touch_shakeout_breakout.md) |
...
- **Next (optional):** Wire `--shakeout-breakout` (any-closed, min_inside=1) into `channel_touch_nightly.py` if live re-arm after exit is wanted; until then nightly stays first-break-and-done. Run `backfill_ib_15m_universe.py` with Gateway up; then dry-run `channel_touch_15m.py`. Bootstrap / max-open on the 15m H5 book; portfolio max-open; ADV-tiered friction.
