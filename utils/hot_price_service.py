"""Install the /hot price hub as an always-on Windows Task Scheduler job.

This is not a crontab.yaml job. CronRunner ticks scheduled bats; live Alpaca
quotes and SELL NOW Telegram need a process that stays up on its own socket.
"""
...
TASK_NAME = r"backTraderTest\HotPriceHub"
...
def default_log_dir(root: Path) -> Path:
    return root / "logs" / "hot_price"


def pid_path(log_dir: Path) -> Path:
    return log_dir / "hot_price.pid"
...
def attach_service_stdio(log_dir: Path) -> Path:
    ...
    log_path = log_dir / "hot_price.log"
    ...
    print("---- hot price server start %s pid=%s ----" % (started, os.getpid()))
    return log_path
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>backTraderTest /hot Alpaca price hub (Flask-Sock) at ws://localhost:%s/ws/hot-candidates. pythonw, no console. Starts at logon and restarts on failure.</Description>
    <URI>\\backTraderTest\\HotPriceHub</URI>
  </RegistrationInfo>
  <Triggers>
    <LogonTrigger>
...
def install_windows_task(
    ...
    script = root / "hot_price_server.py"
    if not pythonw.is_file():
        raise FileNotFoundError("pythonw.exe not found at %s" % pythonw)
    if not script.is_file():
        raise FileNotFoundError("hot_price_server.py not found at %s" % script)
    log_dir = default_log_dir(root)
    log_dir.mkdir(parents=True, exist_ok=True)
    xml_path = log_dir / "hot_price_task.xml"
